...
# Load NASNetMobile dengan pre-trained weights dari ImageNet
base_model = NASNetMobile(weights="imagenet", include_top=False, input_shape=(128, 128, 3))

# Bekukan sebagian layer awal NASNetMobile agar tidak berubah saat training
...
model.save(os.path.join(model_dir, "NASNetMobile.h5"))
print("Model NASNetMobile telah disimpan di 'model/NASNetMobile.h5'")
...
plt.savefig(os.path.join(static_images_dir, 'training_validation_graphs_nasnet.png'))
plt.close()
...
plt.savefig(os.path.join(static_images_dir, 'confusion_matrix_nasnet.png'))
plt.close()
...
plt.savefig(os.path.join(static_images_dir, 'roc_curve_nasnet.png'))
plt.close()
